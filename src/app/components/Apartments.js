...
const ApartmentsContent = ({ userId, showActions = false, favoriteIds, forceRefreshKey, onFavoriteRemoved }) => {
  ...
  const [currentUser, setCurrentUser] = useState(null);
  const [snackbar, setSnackbar] = useState({ open: false, message: '', severity: 'info' });
  const [isCreateUserOpen, setIsCreateUserOpen] = useState(false); // Состояние для модального окна авторизации
  ...
  const toggleFavorite = async (id) => {
    const userProfile = localStorage.getItem('user_profile');
    if (!userProfile) {
      // Показываем модальное окно авторизации вместо снэкбара
      setIsCreateUserOpen(true);
      return;
    }
    try {
