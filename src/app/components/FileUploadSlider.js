...
import DeleteIcon from '@mui/icons-material/Delete';
import ArrowBackIosNewIcon from '@mui/icons-material/ArrowBackIosNew';
import ArrowForwardIiosIcon from '@mui/icons-material/ArrowForwardIos';
import PhoneIcon from '@mui/icons-material/Phone';
import { useSwipeable } from 'react-swipeable';
import ReportIcon from '@mui/icons-material/Report';
import FeedbackIcon from '@mui/icons-material/Feedback';
...
const FileUploadSlider = ({ 
  ...
  const getContainerStyles = () => {
    if (isMobile) {
      return { flexDirection: 'column', width: '100%', maxWidth: '100%' };
    } else if (isTablet) {
      return { flexDirection: 'column', width: '100%', maxWidth: '768px' };
    } else {
      return { flexDirection: 'row', width: '100%', maxWidth: '1200px' };
    }
  };
  ...
  useEffect(() => {
    if (thumbnailsRef.current && localPhotos.length > 0) {
      const thumbWidth = isMobile ? 80 : isTablet ? 100 : 120;
      const gap = 10;
      const scrollPos = currentIndex * (thumbWidth + gap) - (thumbnailsRef.current.offsetWidth / 2) + (thumbWidth / 2);
      thumbnailsRef.current.scrollTo({
        left: scrollPos,
  ...
  }, [currentIndex, localPhotos.length, isMobile, isTablet]);
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ 
        width: isMobile || isTablet ? '100%' : 'calc(100% - 300px)',
        flexShrink: 0,
        position: 'relative',
      }}>
        <Input
