  ...
  const handleFileUpload = useCallback(async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(`❌ Помилка при завантаженні: ${data.error}`);
        return;
      }
  
      const uploadedUrls = Array.isArray(data.url) ? data.url : [data.url];
  
      // Формування фото для локального перегляду
      const uploadedPhotos = uploadedUrls.map((url, index) => ({
        file: null,
        url,
        id: `uploaded-${Date.now()}-${index}-${Math.random().toString(36).substring(2, 5)}`
      }));
  
      const updatedPhotos = [...localPhotos, ...uploadedPhotos];
